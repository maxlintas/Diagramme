...
columns_to_check = ['max. Leistung', 'Spezifische Invest (€/kW) 2023', 'Senk-Aus']
...
x2_data = df['Senk-Aus'].values
y_data = df['Spezifische Invest (€/kW) 2023'].values
def EK_curve(x, a, b, c,d,e):
    x1, x2 = x
...
x2=x2_data
# curve fit
popt, pcov = curve_fit(EK_curve, (x1, x2), y_data,p0=[1, 1, 1, 1, 1])
...
y_data = EK_curve((x1,x2), a, b, c, d, e)
